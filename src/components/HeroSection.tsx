...
const HeroSection = () => {
  ...
  // Background parallax + fade (tracks scroll while hero is in view)
  ...
  const bgOpacity = useTransform(heroScroll, [0, 0.6, 1], [1, 1, 0]);

  // Card fade-out (tracks scroll while card is in view)
  const { scrollYProgress: cardScroll } = useScroll({
    target: cardRef,
    offset: ["start start", "end start"],
  });

  const cardOpacity = useTransform(cardScroll, [0, 0.4], [1, 0]);
  const cardY = useTransform(cardScroll, [0, 0.4], [0, -150]);

  return (
    <header ref={heroRef} className="relative z-10 overflow-hidden">
      {/* Parallax + fading background layer */}
      <motion.div
        style={{ y: bgY, opacity: bgOpacity }}
        className="absolute inset-0 -z-20 bg-[url('/hero.jpg')] bg-cover bg-center"
      />
      ...
      <div className="max-w-330 mx-auto px-20 grid grid-cols-3 gap-28.5">
        ...
        {/* Card wrapper handles scroll-based fade/drift; inner motion.div keeps the entrance pop-in */}
        <motion.div ref={cardRef} style={{ opacity: cardOpacity, y: cardY }} className="col-span-1">
          <motion.div
            variants={{
